...
class BookingRepository @Inject constructor(
    ...
    /**
     * FR-3.1.4: Get scheduled bookings that need to be activated
     */
    suspend fun getScheduledBookingsToActivate(): Result<List<Booking>> {
        return try {
            val currentTime = System.currentTimeMillis()
            val activationTime = currentTime + (10 * 60 * 1000) // 10 minutes from now
            
            val querySnapshot = firestore.collection(BOOKINGS_COLLECTION)
                .whereEqualTo("status", BookingStatus.SCHEDULED)
                .whereLessThanOrEqualTo("scheduledTime", activationTime)
                .whereGreaterThan("scheduledTime", currentTime)
                .get()
                .await()
            
            val scheduledBookings = querySnapshot.documents.mapNotNull { doc ->
                doc.toObject(Booking::class.java)
            }
            
            Result.success(scheduledBookings)
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
    ...
    /**
     * FR-3.1.4: Activate scheduled booking (move to active bookings)
     */
    suspend fun activateScheduledBooking(bookingId: String): Result<Unit> {
        return try {
            ...
            if (!bookingDoc.exists()) {
                return Result.failure(Exception("Scheduled booking not found"))
            }
            
            val booking = bookingDoc.toObject(Booking::class.java)
                ?: return Result.failure(Exception("Failed to parse booking data"))
            
            // Check if booking has been cancelled before activating
            if (booking.status == BookingStatus.CANCELLED) {
                android.util.Log.d("BookingRepository", "Scheduled booking was cancelled before activation - skipping activation")
                return Result.success(Unit)
            }

            // Update status to pending (ready for driver matching)
            firestore.collection(BOOKINGS_COLLECTION)
                .document(bookingId)
                .update("status", BookingStatus.PENDING)
                .await()

            // Create active booking for real-time matching
            activeBookingRepository.createActiveBooking(booking)
    ...
    /**
     * FR-3.1.2: Get real-time estimated travel time using MapBox Matrix API
     * Used for driver ETA updates and live tracking
     */
    suspend fun getRealTimeEstimatedTravelTime(
        origin: BookingLocation,
        destination: BookingLocation
    ): Result<Int> {
        return mapboxRepository.getEstimatedTravelTime(origin, destination)
    }
    
    /**
     * FR-3.1.2: Get enhanced route info with waypoints using Google Directions API
     */
    suspend fun getRouteWithWaypoints(
        pickupLocation: BookingLocation,
        destination: BookingLocation
    ): Result<RouteInfo> {
        return mapboxRepository.getRoute(pickupLocation, destination)
    }
